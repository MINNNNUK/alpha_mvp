"""
id 컬럼을 포함해서 recommend_keyword4 테이블을 생성하는 스크립트
"""
import pandas as pd
import os
...
def create_recommend_with_id(supabase: Client, csv_path: str):
    """id 컬럼을 포함해서 recommend_keyword4 테이블을 생성"""
    try:
        # CSV 파일 읽기 (처음 5행만으로 테스트)
        ...
        logger.info(f"테이블 '{table_name}'에 id를 포함한 데이터 삽입 시도...")
        ...
        try:
            ...
            # id만으로 시도
            try:
                logger.info("id만으로 삽입 시도...")
                id_only_data = [{"id": i + 1} for i in range(len(data))]
                result = supabase.table(table_name).insert(id_only_data).execute()
                logger.info(f"id만으로 {len(id_only_data)}행이 삽입되었습니다.")
                return True
            except Exception as id_error:
                logger.error(f"id만으로도 삽입 실패: {id_error}")
                return False
...
def main():
    ...
    if create_recommend_with_id(supabase, csv_path):
