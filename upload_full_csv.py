"""
전체 CSV 파일을 announcements 테이블에 업로드하는 스크립트
"""
import pandas as pd
import os
...
def upload_full_csv(supabase: Client, csv_path: str):
    """전체 CSV 파일을 announcements 테이블에 업로드"""
    try:
        # CSV 파일 읽기 (청크 단위로)
...
def main():
    ...
    # 전체 CSV 파일 업로드
    if upload_full_csv(supabase, csv_path):
