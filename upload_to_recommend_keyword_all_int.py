"""
recommend_keyword 테이블에 모든 실수 값을 정수로 변환해서 저장하는 스크립트
"""
import pandas as pd
import os
...
def upload_to_recommend_keyword_all_int(supabase: Client, csv_path: str):
    """recommend_keyword 테이블에 모든 실수 값을 정수로 변환해서 저장"""
    try:
        ...
        # 각 레코드에서 NaN 값을 None으로 변환하고 모든 실수를 정수로 변환
        for record in data:
            for key, value in record.items():
                if pd.isna(value) or value == float('inf') or value == float('-inf'):
                    record[key] = None
                # 모든 실수를 정수로 변환
                elif isinstance(value, (int, float)) and key not in ['company_id']:
        ...
        logger.info(f"변환된 데이터 샘플: {mapped_data[0] if mapped_data else 'None'}")
        ...
        try:
            ...
            return upload_full_data_all_int(supabase, csv_path, table_name)
...
def upload_full_data_all_int(supabase: Client, csv_path: str, table_name: str):
    """전체 CSV 데이터를 업로드 (모든 실수를 정수로 변환)"""
    try:
...
def main():
    ...
    if upload_to_recommend_keyword_all_int(supabase, csv_path):
