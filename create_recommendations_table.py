"""
recommendations_keyword_enhanced.csv 파일을 기반으로 새로운 테이블을 생성하고 데이터를 저장하는 스크립트
"""
import pandas as pd
import os
...
def create_recommendations_table(supabase: Client, csv_path: str):
    """recommendations_keyword_enhanced.csv 파일을 기반으로 테이블 생성 및 데이터 저장"""
    try:
        ...
        try:
            ...
            # 다른 테이블명으로 시도
            alternative_tables = ["recommend_keyword4", "recommendations_enhanced", "keyword_recommendations"]
            
            for alt_table in alternative_tables:
                try:
                    logger.info(f"테이블 '{alt_table}'에 데이터 삽입 시도...")
                    result = supabase.table(alt_table).insert(mapped_data).execute()
                    logger.info(f"성공! 테이블 '{alt_table}'에 {len(mapped_data)}행이 삽입되었습니다.")
                    
                    # 전체 데이터 업로드
                    logger.info("전체 데이터 업로드를 시작합니다...")
                    return upload_full_data(supabase, csv_path, alt_table)
                    
                except Exception as alt_error:
                    logger.error(f"테이블 '{alt_table}' 삽입 실패: {alt_error}")
                    continue
...
def main():
    ...
    # recommendations_keyword_enhanced 테이블 생성 및 데이터 저장
    if create_recommendations_table(supabase, csv_path):
        logger.info("recommendations_keyword_enhanced 테이블 생성 및 데이터 저장이 성공적으로 완료되었습니다.")
    else:
        logger.error("테이블 생성 및 데이터 저장에 실패했습니다.")
