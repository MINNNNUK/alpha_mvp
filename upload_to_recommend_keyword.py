"""
recommend_keyword 테이블에 recommendations_keyword_enhanced.csv 데이터를 저장하는 스크립트
"""
import pandas as pd
import os
...
def upload_to_recommend_keyword(supabase: Client, csv_path: str):
    """recommend_keyword 테이블에 CSV 데이터를 저장"""
    try:
        ...
        try:
            ...
            for record in data:
                ...
                mapped_data_no_id.append(mapped_record)
            
            try:
                result = supabase.table(table_name).insert(mapped_data_no_id).execute()
                logger.info(f"id 없이 성공적으로 {len(mapped_data_no_id)}행이 삽입되었습니다.")
                logger.info(f"삽입된 데이터 샘플: {result.data[:2] if result.data else 'None'}")
                
                # 이제 전체 데이터 업로드
                logger.info("전체 데이터 업로드를 시작합니다...")
                return upload_full_data_no_id(supabase, csv_path, table_name)
                
            except Exception as no_id_error:
                logger.error(f"id 없이도 삽입 실패: {no_id_error}")
                return False
...
def upload_full_data_no_id(supabase: Client, csv_path: str, table_name: str):
    """전체 CSV 데이터를 업로드 (id 없이)"""
    try:
        logger.info(f"전체 CSV 데이터를 '{table_name}' 테이블에 업로드 시작 (id 없이)...")
...
def main():
    ...
    if upload_to_recommend_keyword(supabase, csv_path):
